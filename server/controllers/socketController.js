...
const { ALL_LEVELS } = require("../utils/arcadeLevels");
...
function registerSocketHandlers(io) {
  io.on("connection", (socket) => {
    ...
    socket.on("joinRoom", ({ roomId, timerSeconds }) => {
      ...
      if (!existing) {
        ...
        socket.emit("roomFull", {
          message: "Room is full. Try a different Room ID.",
        });
        socket.leave(roomId);
      }
    });
    ...
    // ── JOIN ARCADE ────────────────────────────────────────
    socket.on("joinArcade", ({ levelId }) => {
      const level = ALL_LEVELS.find((l) => l.id === levelId);
      if (!level) {
        socket.emit("invalidMove", { message: "Invalid arcade level." });
        return;
      }

      const roomId = `arcade_${socket.id}`;
      socket.join(roomId);
      const room = createRoom(roomId, socket.id);
      room.vsComputer = true;
      room.isArcade = true;
      room.arcadeLevel = level;
      room.difficulty = level.diff;
      room.timerSeconds = 0;
      room.moveHistory = [];

      // Remove missing piece from board
      _applyArcadeConstraint(room.board, level.missing);
      ...
      console.log(`[A] Arcade L${levelId} "${level.name}" started`);
    });

    // ── MAKE MOVE ──────────────────────────────────────────
    socket.on(
      "makeMove",
      ({ roomId, fromRow, fromCol, toRow, toCol, promotion }) => {
        ...
        const needsPromoChoice =
          p === "p" && ((isWhite && toRow === 0) || (!isWhite && toRow === 7));
...
// ── Apply arcade constraint: remove a piece from white's back rank ──
function _applyArcadeConstraint(board, missingPiece) {
  if (!missingPiece) return;
  const mp = missingPiece.toUpperCase();
  ...
  // For pieces, scan row 7 (white's back rank) and remove the first found
  for (let c = 0; c < 8; c++) {
    if (board[7][c].toUpperCase() === mp) {
      board[7][c] = " ";
      return;
    }
  }
...
function _executeMove(
    ...
    if (!room.initialSnapshot) {
      room.initialSnapshot = {
        ...
        gameState: {
          ...
          enPassant: room.gameState.enPassant
            ? { ...room.gameState.enPassant }
            : null,
        },
        lastMove: null,
      };
  ...
  io.to(roomId).emit("boardUpdate", {
    ...
    isArcade: room.isArcade || false,
    arcadeLevel: room.arcadeLevel || null,
  });
  ...
  if (room.vsComputer && room.turn === "black" && !winner) {
    setTimeout(() => {
      ...
      if (aiP === "p" && Math.abs(aiMove.toRow - aiMove.fromRow) === 2) {
        aiEnPassant = {
          row: (aiMove.fromRow + aiMove.toRow) / 2,
          col: aiMove.fromCol,
        };
      }
      ...
      if (afterStatus === "checkmate") {
        afterWinner = "black";
        room.winner = "black";
      }
      if (afterStatus === "stalemate") {
        afterWinner = "draw";
        room.winner = "draw";
      }
      ...
      if (afterWinner)
        console.log(`[W] CPU ${roomId} — Winner: ${afterWinner}`);
    }, 400);
  }
}
