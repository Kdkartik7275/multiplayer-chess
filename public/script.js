...
function updateTimerDisplay() {
  ...
  myV.className =
    "pc-time" + (mine ? " active" : "") + (timerMyLeft <= 30 ? " low" : "");
  oppV.className =
    "pc-time" + (!mine ? " active" : "") + (timerOppLeft <= 30 ? " low" : "");
}
function fmt(s) {
...
function onSquareClick(row, col) {
  ...
  if (selected) {
    ...
    // ✅ Optimistic animation for instant feedback
    ...
    animatePieceMove(movingPiece, fromRow, fromCol, row, col, () => {
      // Clear optimistic flag after animation completes
      optimisticMove = null;
    });
...
function updateTurnIndicators(turn, winner, status) {
  const myD = document.getElementById("my-turn-dot"),
    oppD = document.getElementById("opp-turn-dot");
  const myL = document.getElementById("my-turn-label"),
    oppL = document.getElementById("opp-turn-label");
  [myD, oppD].forEach((d) => (d.className = "turn-dot"));
  [myL, oppL].forEach((l) => (l.className = ""));
  ...
  if (mine) {
    ...
    oppD.classList.add("active");
    oppL.className = "active-label";
...
function setupPlayerCards() {
  ...
  myAv.className =
    "pc-avatar " + (myColor === "white" ? "white-av" : "black-av");
  ...
  oppAv.className =
    "pc-avatar " + (oppColor === "white" ? "white-av" : "black-av");
...
function addMoveToList(str, color) {
  ...
  if (color === "white") {
    ...
    wC.className = "move-cell white-cell";
    wC.id = `m-w-${num}`;
    wC.textContent = str;
    wC.onclick = () => jumpByCell("w", num);
    const bC = document.createElement("div");
    bC.className = "move-cell black-cell";
    bC.id = `m-b-${num}`;
    bC.textContent = "";
    bC.onclick = () => jumpByCell("b", num);
    row.append(numEl, wC, bC);
    list.appendChild(row);
    movePairs.push({ white: str, black: null, num });
  } else if (movePairs.length > 0) {
    const last = movePairs[movePairs.length - 1];
    last.black = str;
    const bC = document.getElementById(`m-b-${last.num}`);
    if (bC) bC.textContent = str;
  }
...
function jumpByCell(side, pairNum) {
  SFX.click();
  const idx = moveFlat.findIndex(
    (m) => m.pairIdx === pairNum - 1 && m.side === side,
  );
...
function jumpToSnap(idx) {
  ...
  if (m) {
    const cell = document.getElementById(
      `m-${m.side}-${movePairs[m.pairIdx].num}`,
    );
    if (cell) cell.classList.add("active");
  }
}
...
function historyStep(dir) {
  SFX.click();
  if (!isFinite(dir)) {
    if (dir > 0) historyGoLive();
    else if (boardSnapshots.length) jumpToSnap(0);
    return;
  }
  if (!isViewingHistory) {
    if (dir < 0 && boardSnapshots.length > 0)
      jumpToSnap(boardSnapshots.length - 2);
    return;
  }
...
function historyGoLive() {
  ...
  document
    ...
    .forEach((c) => c.classList.remove("active"));
  if (moveFlat.length) {
    const last = moveFlat[moveFlat.length - 1];
    const cell = document.getElementById(
      `m-${last.side}-${movePairs[last.pairIdx].num}`,
    );
    if (cell) cell.classList.add("active");
  }
}
...
function updateGameActButtons() {
  ...
  if (drawBtn) drawBtn.disabled = d || isVsComputer;
}
...
socket.on("waiting", () => {
  document.getElementById("mode-tag").textContent = "Waiting…";
});
socket.on(
  "gameStart",
...
socket.on(
  ...
  ({
    ...
    // ✅ Skip animation if this is our own optimistic move completing
    const isMyOptimisticMove =
      optimisticMove &&
      lm &&
      optimisticMove.fromRow === lm.fromRow &&
      optimisticMove.fromCol === lm.fromCol &&
      optimisticMove.toRow === lm.toRow &&
      optimisticMove.toCol === lm.toCol;
    ...
    if (lm && movedPiece) {
      ...
      addMoveToList(
        GLYPHS[movedPiece] + from + "–" + to,
        isComputerMove ? "black" : turn === "white" ? "black" : "white",
      );
    }
    recordSnapshot(bd, lm, cs);

    if (!isViewingHistory) {
      if (lm && movedPiece && !winner && !isMyOptimisticMove) {
        // ✅ Only animate opponent/AI moves or if optimistic failed
        ...
        // ✅ For our own moves: instantly show final board (optimistic already played)
        renderBoard(bd);
        if (status === "check") {
          showBanner("⚠ Check! Your king is under attack", "check");
          SFX.check();
...
window.addEventListener("load", () => {
  ...
  const menuBgCanvas = document.querySelector(
    "#screen-menu .menu-left .bg-canvas",
  );
  if (menuBgCanvas)
    spawnParticles(menuBgCanvas, 20, ["#d4a843", "#5b8cf5", "#4caf82"]);
